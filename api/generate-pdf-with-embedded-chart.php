<?php
// File: api/generate-pdf-with-embedded-chart.php
// API endpoint to generate PDF report for assessment with embedded chart images
...
// Generate chart as image and embed it in PDF
if (!empty($results)) {
    // Create a temporary image for the chart
    $chartImage = generateMaturityLevelChart($results);
    
    // Add chart to PDF
    $pdf->SetFont('dejavusans', 'B', 12);
    $pdf->Cell(0, 10, 'Visualisasi Maturity Level', 0, 1, 'L');
    $pdf->Ln(5);
    
    // Embed the chart image in the PDF
    $pdf->Image('@'.$chartImage, '', '', 0, 80, '', '', 'T', false, 300, '', false, false, 0, false, false, false);
    $pdf->Ln(85); // Add space after chart
} else {
    ...
    $pdf->Ln(5);
    $pdf->SetFont('dejavusans', '', 10);
    $pdf->Cell(0, 10, 'Tidak ada data untuk ditampilkan dalam visualisasi.', 0, 1, 'L');
    $pdf->Ln(10);
}

// Generate HTML content for the rest of the PDF
$html = generateAcademicReportWithoutChart($assessment, $results, $summary, $groupedResponses);
...
function generateMaturityLevelChart($results) {
    ...
    $grid = imagecolorallocate($image, 200, 200, 200); // Light gray grid
    $text_color = imagecolorallocate($image, 0, 0, 0); // Black text
    $bar_colors = [
        imagecolorallocate($image, 59, 130, 246), // Blue
        imagecolorallocate($image, 16, 185, 129), // Green
        imagecolorallocate($image, 245, 158, 11)  // Yellow
    ];
    ...
    // Draw grid lines
    for ($i = 0; $i <= 5; $i++) {
        $y = $height - 50 - ($i * ($height - 100) / 5);
        imageline($image, 80, $y, $width - 40, $y, $grid);
        imagestring($image, 2, 40, $y - 6, $i, $text_color);
    }
    
    // Draw Y-axis label
    $label = "Tingkat Kematangan";
    $label_width = strlen($label) * 6; // Approximate width
    for ($i = 0; $i < strlen($label); $i++) {
        $char = $label[$i];
        imagestringup($image, 2, 15, $height - 60 - ($i * 10), $char, $text_color);
    }
    
    // Draw X-axis
    imageline($image, 80, $height - 50, $width - 40, $height - 50, $text_color);
    
    // Draw X-axis labels and bars
    $num_results = count($results);
    $bar_width = 60;
    $spacing = ($width - 120) / $num_results - $bar_width;
    ...
    foreach ($results as $result) {
        ...
        $x = 80 + $index * ($bar_width + $spacing);
        $bar_height = ($result['average_score'] / 5) * ($height - 100);
        $y = $height - 50 - $bar_height;
        
        // Draw bar
        $color_index = $index % count($bar_colors);
        imagefilledrectangle($image, $x, $y, $x + $bar_width, $height - 50, $bar_colors[$color_index]);
        imagerectangle($image, $x, $y, $x + $bar_width, $height - 50, $text_color);
        
        // Draw value on top of bar
        $value_str = number_format($result['average_score'], 1);
        $value_width = strlen($value_str) * 6;
        imagestring($image, 2, $x + ($bar_width - $value_width) / 2, $y - 15, $value_str, $text_color);
        
        // Draw label below bar
        $label_width = strlen($processName) * 6;
        imagestring($image, 2, $x + ($bar_width - $label_width) / 2, $height - 35, $processName, $text_color);
        
        $index++;
    }
    
    // Draw chart title
    $title = "Visualisasi Maturity Level per Proses";
    $title_width = strlen($title) * 8; // Approximate width for larger font
    imagestring($image, 3, ($width - $title_width) / 2, 10, $title, $text_color);
    ...
    // Return image data for TCPDF
    return $image_data;
}

function generateAcademicReportWithoutChart($assessment, $results, $summary, $groupedResponses) {
    $html = '
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <!-- Section 3: Detail Hasil Assessment -->
        <div class="section">
            <div class="section-title">3. Detail Hasil Assessment</div>';
